...
SEARCH_SELECTION_DEFECT_LOG_PATH = log_dir / "search_selection_defect.log"
OUT_OF_BOUND_DEFECT_LOG_PATH = log_dir / "out_of_bound_defect.log"

TITLE_IMAGE_DEFECT_JSON_PATH = log_dir / "title_image_defect.json"
SEARCH_SELECTION_DEFECT_JSON_PATH = log_dir / "search_selection_defect.json"
...
def process_defect_logs():
    ...
    if card_info:
        ...
        search_selection_defect_lines = get_file_contents(SEARCH_SELECTION_DEFECT_LOG_PATH)
        ...
        for card in card_info:
            name = card["name"]
            if title_image_defect_lines and any(name in decode_str(line) or line for line in title_image_defect_lines):
                title_image_defect_json.append(card)
            if search_selection_defect_lines and any(name in decode_str(line) or line for line in search_selection_defect_lines):
                search_selection_defect_json.append(card)
            if out_of_bound_defect_lines and any(name in decode_str(line) or line for line in out_of_bound_defect_lines):
...
def process_debug_log():
    ...
    if debug_lines:
        ...
        for line in debug_lines:
            ...
            if SEARCH_SELECTION_DEFECT in line:
                search_selection_defect_lines.append(line)    
        ...
        writelines_to_file(SEARCH_SELECTION_DEFECT_LOG_PATH, search_selection_defect_lines)  
